...
import { useGetChannelActivityRegistrationCreateMutation } from "../../features/stasis/StasisActivity"; // TESTING APPROACH 1
import { useGetChannelsActivityQuery } from "../../features/stasis/StasisActivity";
import { useDeleteChannelMutation } from "../../features/asterisk/asteriskApiSlice";
...
const DialPadRtc = ({colors}) => {
  ...
  const [ channelRegistration ] = useGetChannelActivityRegistrationCreateMutation()
  const { data: activeChannels, refetch } = useGetChannelsActivityQuery();
  const [channelDelete ] = useDeleteChannelMutation()

  useEffect(() => {
    console.log('useEffect > inCallStatus', inCall)
    dispatch(inCallStatus(inCall));
  }, [dispatch, setInCall, inCall]);

  useEffect(() => {
    console.log('useEffect > registeredStatus', registered)
    dispatch(registeredStatus(registered));
  }, [dispatch, setRegistered, registered]);

  useEffect(() => {
    dispatch(callStartedTime(serializableDate));
  }, [dispatch, serializableDate]);
  ...
  const login = () => {
    try {
      ...
      ua.on("connected", function (e) {
        ...
        channelRegistration(`${user["user"]["extension"]}`) // TESTING APPROACH 1
      });
  ...
  const HandleCloseChannel = () => {
    console.log('activeChannels+++', activeChannels && activeChannels)
    console.log('user.user.extension+++', user.user.extension)
    ...
    // channelDelete(currentExtension['channel_id']) . // IDENTIFY WHY IS CRASHING STASIS
  }
  ...
  // console.log('inCallStatusSelector.valueInCallStatus---', inCallStatusSelector.valueInCallStatus)
  // console.log('inCallStatusSelector.valueRegisteredStatus---', inCallStatusSelector.valueRegisteredStatus)
  ...
  return (
    <div>
      ...
      >
        <Box sx={{ mb: 2 }}>
          {!inCallStatusSelector.valueRegisteredStatus && (
            <Button
              sx={{
          ...
          {inCallStatusSelector.valueRegisteredStatus && !inCallStatusSelector.valueInCallStatus && (
            <Button
              sx={{
        ...
        <Grid>
          ...
          { !inCallStatusSelector.valueInCallStatus && !inCallStatusSelector.valueRegisteredStatus && (
            <Button
            sx={{
            ...
            type="submit"
            variant="contained"
            disabled={!inCallStatusSelector.valueRegisteredStatus}
          >
            <Close>{}</Close>
          </Button>
          )}
          ...
          { inCallStatusSelector.valueRegisteredStatus && inCallStatusSelector.valueInCallStatus && (
            ...
            type="submit"
            variant="contained"
            // onClick={() => HandleCloseChannel()} - NEED TO FIX TEH MALFORMED STRING IN PYTHON/STASIS
            onClick={() => HandleLogOut()}
            disabled={!inCallStatusSelector.valueRegisteredStatus}
          >
            <Close>{}</Close>
          ...
          <Button
            ...
            variant="contained"
            onClick={() => handleCall()}
            disabled={!inCallStatusSelector.valueRegisteredStatus || inCallStatusSelector.valueInCallStatus}
          >
            <Call>{}</Call>
          </Button>
        </Grid>
