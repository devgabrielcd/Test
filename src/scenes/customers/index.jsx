...
import CustomersCardView from "../../components/customers/CustomersCardView";
import CustomersMapView from "../../components/customers/CustomersMapView";
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2} sx={{ marginTop: '-30px', marginBottom: '-40px' }}>
        ...
        <Grid item xs={6} md={5}>
          ...
          <Box sx={{ mt: 16, display: 'flex', justifyContent: 'right'}}>
            <Tooltip title="Next Page" arrow classes={{ tooltip: classes.customTooltip }} placement='top'>
              <IconButton sx={{ color: colors.grey[400] }} aria-label="grid view">
                <NavigateBeforeIcon />
              </IconButton>
            </Tooltip>
            <Tooltip title="Previous Page" arrow classes={{ tooltip: classes.customTooltip }} placement='top'>
            ...
            <Tooltip title="Filter" arrow classes={{ tooltip: classes.customTooltip }} placement='top'>
              <IconButton sx={{ color: colors.grey[400] }} aria-label="grid view">
                <FilterAltIcon />
              </IconButton>
            </Tooltip>
            <Tooltip title="List View" arrow classes={{ tooltip: classes.customTooltip }} placement='top'>
            <IconButton sx={{ color: viewMode === 'list' ? (colors.icons[400]) : (colors.grey[400]) }} aria-label="list view"  onClick={() => setViewMode('list')}>
              <ReorderIcon />
            </IconButton>
            </Tooltip>
            <Tooltip title="Card View" arrow classes={{ tooltip: classes.customTooltip }} placement='top'>
              <IconButton sx={{ color: viewMode === 'grid' ? (colors.icons[400]) : (colors.grey[400]) }} aria-label="grid view"  onClick={() => setViewMode('grid')}>
                <GridViewIcon />
              </IconButton>
            </Tooltip>
            <Tooltip title="Map View" arrow classes={{ tooltip: classes.customTooltip }} placement='top'>
      ...
      <Fade in={viewMode === 'list'} timeout={500}>
        <Box>
          {viewMode === 'list' ? <CustomersListView colors={colors} customers={customers} isLoading={isLoading} isSuccess={isSuccess}/> : null}
        </Box>
      </Fade>
      <Fade in={viewMode === 'grid'} timeout={500}>
        <Box>
          {viewMode === 'grid' ? <CustomersCardView colors={colors} customers={customers} isLoading={isLoading} isSuccess={isSuccess}/> : null}
        </Box>
      </Fade>
      <Fade in={viewMode === 'map'} timeout={500}>
        <Box>
          {viewMode === 'map' ? <CustomersMapView colors={colors} customers={customers} isLoading={isLoading} isSuccess={isSuccess}/> : null}
        </Box>
      </Fade>
      </Box>
      {/* <DraggableCreateUser /> */}
